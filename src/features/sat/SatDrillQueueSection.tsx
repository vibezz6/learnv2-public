...
import {
  formatDrillCooldownLabel,
  getDrillCooldownRows,
  getDrillQueue,
  markSkillDrilled,
} from "@/lib/satDrillQueue";
...
export function SatDrillQueueSection({ subject }: { subject: Subject }) {
  ...
  if (queue.length === 0 && cooldownRows.length === 0) return null;
  ...
  return (
    <Card variant="default" density="normal" className="min-w-0">
      ...
      {cooldownRows.length > 0 ? (
        <div className={queue.length > 0 ? "mt-4 border-t border-[var(--rule)] pt-3" : "mt-3"}>
          <p className="text-xs font-medium text-[var(--text-muted)]">On cooldown</p>
          <ul className="mt-2 space-y-2">
            {cooldownRows.map((row) => (
              <li
                key={row.skillId}
                className="flex items-center justify-between gap-2 rounded-[var(--radius)] border border-[var(--rule)] bg-[var(--bg-panel)] px-3 py-2 text-sm opacity-80"
              >
                <span className="font-medium text-[var(--text-muted)]">{row.label}</span>
                <span className="shrink-0 text-xs text-[var(--text-subtle)]">
                  {formatDrillCooldownLabel(row.hoursLeft)}
                </span>
              </li>
            ))}
          </ul>
